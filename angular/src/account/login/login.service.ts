...
import {
    AuthenticateModel,
    AuthenticateResultModel, ErpSignInServiceProxy,
...
export class LoginService {
    ...
    constructor(
        ...
        private _erpServiceService: ErpSignInServiceProxy
    ) {
        this.clear();
    }
    ...
    erpAuthenticate(token: string, code: string ): void {
        this._erpServiceService.signInUser(token, code).subscribe(user => {

            if (user.username) {
                this.errorErpAuthenticate=false;
                this.authenticateModel.userNameOrEmailAddress = user.username;
                this.authenticateModel.password = user.oneTimePassword;

                this.authenticate();
            }else{
                this.errorErpAuthenticate=true;
            }
        });
    }
