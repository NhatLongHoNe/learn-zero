            ...
            List<ImportUserDto> invalidUsers)
        {
            ...
            else
            {
                await _appNotifier.SendMessageAsync(
                    args.User,
                    new LocalizableString("AllUsersSuccessfullyImportedFromExcel", AbpZeroTemplateConsts.LocalizationSourceName),
                    null,
            ...
            using (var uow = _unitOfWorkManager.Begin())
            {
                using (CurrentUnitOfWork.SetTenantId(args.TenantId))
                {
                    AsyncHelper.RunSync(() => _appNotifier.SendMessageAsync(
                        args.User,
                        new LocalizableString("FileCantBeConvertedToUserList", AbpZeroTemplateConsts.LocalizationSourceName),
                        null,
